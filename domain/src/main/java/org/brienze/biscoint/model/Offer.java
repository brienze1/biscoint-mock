...
public class Offer {
    ...
    public Offer(OfferRequest offerRequest, String apiKey, BigDecimal bitcoinUnitaryValue) {
        ...
        setExpiresAt(LocalDateTime.now().plus(Duration.ofSeconds(15)));
        setApiKeyId(apiKey);

        if (quotedInBrl) {
            setQuoteAmount(offerRequest.getAmount().divide(bitcoinUnitaryValue, 8, RoundingMode.HALF_UP));
        } else {
    ...
    public void confirm() {
        Validators.validateTime(LocalDateTime.now(), expiresAt, "Offer expired.");
